...
class BaselineDetector:
    ...
    def save_results(self, image_path: str, start_points: np.ndarray, end_points: np.ndarray, 
                    baselines: List[np.ndarray], polygons: List[Optional[np.ndarray]], output_path: str):
...
def main():
    ...
    # Save results
    output_path = output_dir / f"{Path(args.image_path).stem}_results.json"
    detector.save_results(args.image_path, start_points, end_points, baselines, polygons, str(output_path))
    
    # Create visualization
    vis_image = image.copy()
    
    # Draw polygons in semi-transparent green first (so baselines will be on top)
    for polygon in polygons:
        if polygon is not None:
            # Reshape for OpenCV
            polygon = polygon.reshape(-1, 1, 2).astype(np.int32)
            # Create a copy of the image for the polygon
            ...
            cv2.polylines(vis_image, [polygon], True, (0, 200, 0), 1)
